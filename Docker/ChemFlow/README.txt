...
##  GeneratedInvariants directory

Contains the invariants previously generated in our study.

## Subjects directory

Contains the subject CRN files used with the ChemFlow program.

## ../SummaryProgram directory

Contains the python script which generates Table 4 in our paper. This analysis is done on the data in the `SimulationData` directory.

## ../SimulationData directory

Contains the output of the Matlab Simbiology simulations of subject CRNs in our test suite.
These files are evaluated to generate a table from the paper.

## Files

- The source code for the ChemFlow program. This is compiled when running the docker build command.
- runAllInvariants.sh is a script to run all subjects and store the output to text files based on the input CRN located in this directory.
